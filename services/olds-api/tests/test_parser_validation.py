...
def test_nontext_interactive_is_not_a_false_article_body_failure(
    ...
    connection.execute(
        ...
        INSERT INTO parser_validation_config(
            ...
            VALUES (2020, 1, 'test', 'nyt-parser/0.8.70', 2, 'now')
        """
    )
    connection.execute(
        """
...
def test_axios_internal_fixture_does_not_fill_article_validation_target(
    ...
    connection.execute(
        ...
        INSERT INTO parser_validation_config(
            sample_year, target_size, seed, parser_version, qa_revision,
            updated_at
        ) VALUES (2017, 1, 'test', 'axios-parser/0.1.22', 4, 'now')
        """
    )
    connection.execute(
        """
...
def test_malformed_axios_url_alias_does_not_fill_validation_target(
    ...
    connection.execute(
        ...
        INSERT INTO parser_validation_config(
            sample_year, target_size, seed, parser_version, qa_revision,
            updated_at
        ) VALUES (2025, 1, 'test', 'axios-parser/0.1.22', 4, 'now')
        """
    )
    connection.execute(
        """
...
def test_nyt_print_utility_entry_is_screened_from_article_cohort(
    ...
    connection.execute(
        ...
        INSERT INTO parser_validation_config(
            sample_year, target_size, seed, parser_version, qa_revision,
            updated_at
            ) VALUES (?, 1, 'test', 'nyt-parser/0.8.70', 2, 'now')
        """,
        (sample_year,),
    )
    connection.execute(
