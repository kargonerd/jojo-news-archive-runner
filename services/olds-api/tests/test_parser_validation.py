...
def test_qa_revision_change_replays_without_replacing_cohort(
    ...
    assert refreshed["parserVersion"] == "wsj-parser/0.8.59"
...
def test_ft_subscribe_shell_is_excluded_from_article_cohort(
    ...
    connection.execute(
        ...
        INSERT INTO parser_validation_config(
            sample_year, target_size, seed, parser_version, qa_revision,
            updated_at
        ) VALUES (2022, 1, 'test', 'ft-parser/0.8.51', 2, 'now')
        """
    )
    connection.execute(
        """
...
def test_npr_short_audio_shell_is_excluded_from_article_cohort(
    ...
    connection.execute(
        ...
        INSERT INTO parser_validation_config(
            sample_year, target_size, seed, parser_version, qa_revision,
            updated_at
        ) VALUES (2014, 1, 'test', 'npr-parser/0.1.51', 1, 'now')
        """
    )
    connection.execute(
        """
...
def test_wsj_media_unsupported_shell_is_excluded_from_article_cohort(
    ...
    connection.execute(
        ...
        INSERT INTO parser_validation_config(
            sample_year, target_size, seed, parser_version, qa_revision,
            updated_at
        ) VALUES (2018, 1, 'test', 'wsj-parser/0.8.59', 3, 'now')
        """
    )
    connection.execute(
        """
...
def test_caixin_photo_desk_does_not_fill_article_validation_target(
    ...
    connection.execute(
        ...
        INSERT INTO parser_validation_config(
            sample_year, target_size, seed, parser_version, qa_revision,
            updated_at
            ) VALUES (2010, 1, 'test', 'caixin-parser/0.1.13', 1, 'now')
        """
    )
    connection.execute(
        """
